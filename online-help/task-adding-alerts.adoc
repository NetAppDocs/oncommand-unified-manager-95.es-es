...
* Debe haber configurado los ajustes de notificación, como la dirección de correo electrónico de usuario, el servidor SMTP y el host de captura SNMP, con el fin de permitir que el servidor de Unified Manager utilice estos ajustes para enviar notificaciones a los usuarios cuando se genera un evento.
...
* Si desea que un script se ejecute según el evento, debe haber añadido el script a Unified Manager mediante la página Management/Scripts.
...
Puede crear una alerta directamente desde la página de detalles Event después de recibir un evento además de crear una alerta desde la página Configuration/Alerting, tal y como se describe aquí.
...
. En el menú Select Script to Execute (Seleccionar script para ejecutar), seleccione *Test* script .
. Haga clic en *Guardar*.
